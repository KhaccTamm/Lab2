...
exports.create = async (req, res, next) => {
    if(!req.body?.name){
        return next(
            new BadRequestError(400, "Name can not be empty")
        );
    }
    try {
        ...
        return next(
            new BadRequestError(500,"An error occurred while creating the contacts")
        );
    }
};
...
exports.findAll = async(req, res, next) => {
    ...
    try {
        ...
        return next(
            new BadRequestError(500, "An error occurred while retrieving contact")
        );
    }
...
exports.findOne = async(req, res, next) => {
    ...
    try {
        ...
        return next(
            new BadRequestError(500, `Error retrieving contact with td=${req.params.id}`)
        );
    }
};
...
exports.update = async (req,res,next) => {
    ...
    try {
        ...
        if (!document) {
            return next(
                new BadRequestError(404, "Contact not found")
            );
        }
